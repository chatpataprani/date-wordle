...
function finish(d,win){done=true;const used=Math.min(row+1,MAX),box=document.createElement('div');box.className='result';box.innerHTML=win?'<b>💌 SOLVED!</b><div class="small">'+used+'/'+MAX+' tries</div>':'<b>💀 GAME OVER</b><div class="small">All '+MAX+' chances used</div>';if(win&&d.message)box.innerHTML+='<div style="margin-top:8px">💌 '+d.message+'</div>';if(win&&d.reward)box.innerHTML+='<div style="margin-top:8px">🎁 '+d.reward+'</div>';document.body.appendChild(box);const copy=document.createElement('button');copy.textContent='📋 COPY RESULT';copy.onclick=()=>navigator.clipboard?.writeText(shareText());box.appendChild(copy)}
