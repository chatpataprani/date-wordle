...
function makeSlug(value) {
  return clean(value, 24).toLowerCase().replace(/[^a-z0-9-]/g, '').replace(/-+/g, '-').replace(/^-|-$/g, '');
}
function esc(value) {
...
function home() {
  ...
  <div class="panel">
    ...
    <details class="options"><summary>OPTIONAL EXTRAS ⚙️</summary>
      ...
      <div class="option">Theme<select id="theme"><option>classic</option><option>rose</option><option>neon</option><option>mint</option><option>amoled</option></select></div>
...
<script>
(function(){
  const $=id=>document.getElementById(id), btn=$('createBtn'), out=$('out');
  ...
  btn.addEventListener('click',async function(){
    ...
    out.textContent='';
    if(!name){out.textContent='Please enter your name first.';return}
    if(!/^[A-Z]{3,12}$/.test(word)){out.textContent='Secret word must be 3-12 letters.';return}
    btn.disabled=true; btn.textContent='CREATING...';
    try{
      const payload={name,word,mode:$('mode').value,theme:$('theme').value,roastLevel:$('roast').value,hintsEnabled:$('hints').checked,timerEnabled:$('timer').checked,reactionsEnabled:$('reactions').checked,slug:$('slug').value,message:$('message').value,reward:$('reward').value};
      ...
      out.innerHTML='<div class="small">💌 '+String(data.name).replace(/[&<>"']/g,'')+'\'s link</div><div class="card" style="margin-top:8px;word-break:break-all">'+String(data.link).replace(/[&<>"']/g,'')+'</div><button type="button" id="copyLink">COPY LINK</button><a href="'+data.link+'" style="display:block;text-align:center;margin-top:10px">OPEN GAME 🎮</a>';
      $('copyLink').addEventListener('click',async()=>{try{await navigator.clipboard.writeText(data.link);$('copyLink').textContent='COPIED ✓'}catch{$('copyLink').textContent='COPY FAILED'}});
...
function soloPage() {
  ...
  <button type="button" data-d="easy">🟢 EASY</button><button type="button" data-d="medium">🟡 MEDIUM</button><button type="button" data-d="hard">🔴 HARD</button><div id="msg" class="msg"></div>
  </div><a href="/">← CREATE A LINK</a>
<script>
document.querySelectorAll('[data-d]').forEach(btn=>btn.addEventListener('click',async()=>{const msg=document.getElementById('msg');document.querySelectorAll('[data-d]').forEach(b=>b.disabled=true);msg.textContent='picking a word... 👀';try{const r=await fetch('/api/solo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({difficulty:btn.dataset.d})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not start solo mode');location.href=d.link}catch(e){msg.textContent='❌ '+e.message;document.querySelectorAll('[data-d]').forEach(b=>b.disabled=false)}}));
</script></body></html>`;
}

function playPage(g) {
  const name = g.solo ? 'Solo Mode' : g.name;
  const reactions = g.reactionsEnabled ? '<button type="button" data-react="😭">😭</button><button type="button" data-react="🗿">🗿</button><button type="button" data-react="🤯">🤯</button><button type="button" data-react="❤️">❤️</button>' : '';
  const hint = g.hintsEnabled ? '<button type="button" id="hint">💡 HINT · 1 TRY</button>' : '';
  return `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>DATE ME — PLAY</title><style>${CSS}</style></head><body>
...
<div class="tools">${hint}<button type="button" id="share">📤 SHARE GAME</button></div><div class="tools">${reactions}</div><div id="reactionMsg" class="small"></div>
<script>
...
for(const c of 'QWERTYUIOP')key(k1,c,()=>press(c));
for(const c of 'ASDFGHJKL')key(k2,c,()=>press(c));
key(k3,'ENTER',submit,true);for(const c of 'ZXCVBNM')key(k3,c,()=>press(c));key(k3,'DEL',back,true);
...
async function submit(){if(done||col!==LEN)return;const guess=board[row].join('');setKeys(true);try{const r=await fetch('/api/guess/'+encodeURIComponent(ID),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({guess})});const d=await r.json().catch(()=>({error:'Invalid server response'}));if(!r.ok)throw new Error(d.error||'Guess failed');results[row]=d.result;d.result.forEach((state,i)=>{document.getElementById('cell-'+row+'-'+i).classList.add(state);const k=keys[guess[i]];if(k){if(state==='correct')k.className='key correct';else if(state==='present'&&!k.classList.contains('correct'))k.className='key present';else if(state==='absent'&&!k.classList.contains('correct')&&!k.classList.contains('present'))k.className='key absent'}});document.getElementById('msg').textContent=d.feedback||feedback(d.result);if(d.solved||d.exhausted){finish(d,d.solved);return}row++;col=0;setKeys(false)}catch(e){document.getElementById('msg').textContent='❌ '+e.message;setKeys(false)}}
...
function makeCard(win,used,streak){const card=document.createElement('div');card.className='sharecard';let rows='';for(const r of results.slice(0,used)){if(!r)continue;rows+='<div class="sharerow" style="grid-template-columns:repeat('+LEN+',1fr)">'+r.map(s=>'<div class="sharecell" style="background:'+(s==='correct'?'#4caf6d':s==='present'?'#c9a227':'#3b3b42')+'"></div>').join('')+'</div>'}card.innerHTML='<h2>'+(win?'💌 '+NAME+' SOLVED DATE WORDLE':'💀 '+NAME+' GOT COOKED')+'</h2><div class="small">'+(win?used+'/'+MAX+' tries · 🔥 '+streak+' streak':'X/'+MAX+' tries')+'</div><div class="sharegrid">'+rows+'</div>'+(win&&MESSAGE?'<div>💌 '+esc(MESSAGE)+'</div>':'')+(win&&REWARD?'<div style="margin-top:6px">🎁 '+esc(REWARD)+'</div>':'')+'<div class="shareplay">🎮 PLAY THIS GAME<br><small>'+esc(location.href)+'</small></div>';document.body.appendChild(card);return card}
...
function finish(d,win){done=true;const old=Number(localStorage.getItem('dateStreak')||0),streak=win?old+1:0;localStorage.setItem('dateStreak',streak);const used=Math.min(row+1,MAX),box=document.createElement('div');box.className='result';box.innerHTML=win?'<b>💌 SOLVED!</b><div class="small">'+used+'/'+MAX+' tries · 🔥 '+streak+' streak</div>':'<b>💀 GAME OVER</b><div class="small">All '+MAX+' chances used 😭</div>';if(win&&d.message)box.innerHTML+='<div style="margin-top:8px">💌 '+esc(d.message)+'</div>';if(win&&d.reward)box.innerHTML+='<div style="margin-top:8px">🎁 '+esc(d.reward)+'</div>';document.body.appendChild(box);const card=makeCard(win,used,streak);const shareBtn=document.createElement('button');shareBtn.type='button';shareBtn.textContent='🖼️ SHARE RESULT CARD';shareBtn.onclick=()=>shareCard(card);const copyBtn=document.createElement('button');copyBtn.type='button';copyBtn.textContent='📋 COPY RESULT';copyBtn.onclick=()=>{navigator.clipboard?.writeText(NAME+' · '+(win?used+'/'+MAX:'X/'+MAX)+'\\n'+results.slice(0,used).filter(Boolean).map(a=>a.map(s=>s==='correct'?'🟩':s==='present'?'🟨':'⬛').join('')).join('\\n')+'\\n🎮 PLAY: '+location.href);copyBtn.textContent='COPIED ✓'};box.appendChild(shareBtn);box.appendChild(copyBtn);card.scrollIntoView({behavior:'smooth',block:'center'})}
async function hint(){if(done||hintUsed)return;const p=prompt('Which letter position? (1-'+LEN+')');if(p===null)return;const index=Number(p)-1;if(!Number.isInteger(index)||index<0||index>=LEN)return;try{const r=await fetch('/api/hint/'+encodeURIComponent(ID),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({index})});const d=await r.json();if(!r.ok)throw new Error(d.error);hintUsed=true;document.getElementById('msg').textContent='💡 Position '+(index+1)+' is '+d.letter+' · 1 try used';row++;col=0;if(row>=MAX)document.getElementById('msg').textContent+=' · no tries left'}catch(e){document.getElementById('msg').textContent='❌ '+e.message}}
...
app.post('/api/create', (req,res) => {
  const word=clean(req.body?.word,12).toUpperCase();
...
app.post('/api/guess/:id', (req,res) => {
  ...
  if(g.solved||g.exhausted)return res.status(400).json({error:'This game is already finished'});
  ...
  try{save(dbUpdate(g))}catch(e){return res.status(500).json({error:'Could not save your guess'})}
  res.json({ok:true,result,solved:g.solved,exhausted:g.exhausted,feedback:g.solved?'YOOOO YOU GOT IT 😭🔥':result.filter(x=>x==='correct').length>=g.word.length-1?ALMOST[Math.floor(Math.random()*ALMOST.length)]:'',message:g.solved?g.message:'',reward:g.solved?g.reward:''});
});

function dbUpdate(updated){const games=db();games[updated.id]=updated;return games}
...
app.post('/api/hint/:id', (req,res) => {
  ...
  g.hintUsed=true;try{save(dbUpdate(g))}catch(e){return res.status(500).json({error:'Could not save hint'})}res.json({ok:true,letter:g.word[index]});
});
...
app.post('/api/reaction/:id', (req,res) => {
  ...
  g.reaction=req.body.reaction;try{save(dbUpdate(g))}catch(e){return res.status(500).json({error:'Could not save reaction'})}res.json({ok:true});
});
