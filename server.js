...
async function shareGame(){const text=NAME+' — DATE ME\\n'+results.filter(Boolean).map(a=>a.map(s=>s==='correct'?'🟩':s==='present'?'🟨':'⬛').join('\\n')).join('\\n')+'\\n\\n💡 Hint used: '+(hintUsed?'YES':'NO')+'\\n🎮 Play: '+location.href;try{const pngUrl='/api/result-card/'+encodeURIComponent(ID)+'.png';if(navigator.share){try{const response=await fetch(pngUrl);if(!response.ok)throw Error('PNG unavailable');const blob=await response.blob();const file=new File([blob],'date-wordle-result.png',{type:'image/png'});if(!navigator.canShare||navigator.canShare({files:[file]})){await navigator.share({title:'DATE ME — Result',text,url:location.href,files:[file]});return}}catch(e){if(e.name==='AbortError')return}await navigator.share({title:'DATE ME — Result',text,url:location.href});return}const a=document.createElement('a');a.href=pngUrl;a.download='date-wordle-result.png';document.body.appendChild(a);a.click();a.remove();try{await navigator.clipboard.writeText(text)}catch{}msg.textContent='🖼️ Result PNG downloaded · result text copied'}}
