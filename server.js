...
<details><summary>OPTIONAL EXTRAS ⚙️</summary>
...
<label class="small">Hints · costs 1 try <input id="hints" type="checkbox"></label>
<label class="small">Speed timer <input id="timer" type="checkbox"></label>
<label class="small">Reactions <input id="reactions" type="checkbox"></label>
<input id="slug" placeholder="CUSTOM LINK NAME (optional)" maxlength="24">
...
let row=0,col=0,done=false,start=Date.now(),hintUsed=false,results=[];
...
function makeGrid(){for(let r=0;r<MAX;r++){const rr=document.createElement('div');rr.className='row';rr.style.gridTemplateColumns='repeat('+LEN+',1fr)';for(let c=0;c<LEN;c++){const x=document.createElement('div');x.className='cell';x.id='cell-'+r+'-'+c;rr.appendChild(x)}grid.appendChild(rr)}}
...
function makeCard(win,used){const card=document.createElement('div');card.className='sharecard';let rows='';for(const a of results.slice(0,used)){rows+='<div class="sharerow" style="grid-template-columns:repeat('+LEN+',1fr)">'+a.map(s=>'<div class="sharecell" style="background:'+(s==='correct'?'#4caf6d':s==='present'?'#c9a227':'#3b3b42')+'"></div>').join('')+'</div>'}card.innerHTML='<h2>'+(win?'💌 '+NAME+' SOLVED DATE WORDLE':'💀 '+NAME+' GOT COOKED')+'</h2><div class="small">'+(win?used+'/'+MAX+' tries':'X/'+MAX+' tries')+'</div><div class="sharegrid">'+rows+'</div>'+(win&&MESSAGE?'<div>💌 '+MESSAGE+'</div>':'')+(win&&REWARD?'<div style="margin-top:6px">🎁 '+REWARD+'</div>':'')+'<div class="shareplay">🎮 PLAY THIS GAME<br><small>'+location.href+'</small></div>';document.body.appendChild(card);return card}
async function shareCard(card){const canvas=document.createElement('canvas'),ctx=canvas.getContext('2d');canvas.width=900;canvas.height=700;ctx.fillStyle='#0e0e12';ctx.fillRect(0,0,900,700);ctx.fillStyle='#fff';ctx.font='bold 46px monospace';ctx.fillText(card.querySelector('h2').textContent,40,65);ctx.font='26px monospace';ctx.fillStyle='#aaa';ctx.fillText(card.querySelector('.small').textContent,40,105);let y=145;for(const a of results.slice(0,MAX)){for(let i=0;i<a.length;i++){ctx.fillStyle=a[i]==='correct'?'#4caf6d':a[i]==='present'?'#c9a227':'#3b3b42';ctx.fillRect(40+i*62,y,54,54)}y+=62}ctx.fillStyle='#ff5d8f';ctx.font='bold 24px monospace';ctx.fillText('🎮 PLAY THIS GAME',40,y+25);ctx.fillStyle='#aaa';ctx.font='18px monospace';ctx.fillText(location.href,40,y+55);const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw Error('Could not create image');const file=new File([blob],'date-wordle-result.png',{type:'image/png'});if(navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({title:'DATE ME RESULT',text:location.href,files:[file]});return}const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='date-wordle-result.png';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
function finish(d,win){done=true;const used=Math.min(row+1,MAX),box=document.createElement('div');box.className='result';box.innerHTML=win?'<b>💌 SOLVED!</b><div class="small">'+used+'/'+MAX+' tries</div>':'<b>💀 GAME OVER</b><div class="small">All '+MAX+' chances used</div>';if(win&&d.message)box.innerHTML+='<div style="margin-top:8px">💌 '+d.message+'</div>';if(win&&d.reward)box.innerHTML+='<div style="margin-top:8px">🎁 '+d.reward+'</div>';document.body.appendChild(box);const card=makeCard(win,used);const share=document.createElement('button');share.textContent='🖼️ SHARE RESULT CARD';share.onclick=()=>shareCard(card).catch(e=>msg.textContent='❌ '+e.message);const copy=document.createElement('button');copy.textContent='📋 COPY RESULT';copy.onclick=()=>navigator.clipboard?.writeText(shareText());box.append(share,copy)}
async function hint(){if(done||!HINTS||hintUsed)return;try{const p=prompt('Which letter position? (1-'+LEN+')');if(p===null)return;const index=Number(p)-1;if(!Number.isInteger(index)||index<0||index>=LEN)throw Error('Invalid position');const r=await fetch('/api/hint/'+encodeURIComponent(ID),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({index})});const d=await r.json();if(!r.ok)throw Error(d.error);hintUsed=true;msg.textContent='💡 Position '+(index+1)+' is '+d.letter+' · 1 try used';if(row<MAX){row++;col=0}}catch(e){msg.textContent='❌ '+e.message}}
